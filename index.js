...
+ '<script>var vals=' + JSON.stringify(valores) + '.map(function(v){return parseFloat(v.toString().replace(",","."));});var ctx=document.getElementById("pizza").getContext("2d");new Chart(ctx,{type:"bar",indexAxis:"y",data:{labels:' + JSON.stringify(categorias) + ',datasets:[{data:vals,backgroundColor:' + JSON.stringify(cores.slice(0, categorias.length)) + ',borderWidth:0,borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return" R$ "+c.raw.toLocaleString("pt-BR",{minimumFractionDigits:2});}}}},scales:{x:{ticks:{callback:function(v){return"R$ "+v.toLocaleString("pt-BR",{minimumFractionDigits:2});},font:{size:10}},grid:{display:false}},y:{grid:{display:false},ticks:{font:{size:11}}}}}});<\/script>';
